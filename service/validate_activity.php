...
const INVALID_CHARS = "<>,\"`@\/\\\\|{}\[\]()*$%#?=:;";
$FIELDS = array_keys(Activity::INPUTS_MAP);
unset($FIELDS[array_search('id', $FIELDS)]);
unset($FIELDS[array_search('fk-module', $FIELDS)]);
        ...
        if (isset($_POST[$field])) {
            ...
            switch ($field) {
                case 'title':
                    ...
                    break;
                case 'index':
                    if (!isset($_POST['fk-module']) || (int) $_POST['fk-module'] == 0) {
                        break;
                    }
                    $activities = Activity::findByCondition($conn, new Activity(), 'fk_module', $_POST['fk-module']);
                    $check = ((int) $value) > 0;
                    foreach ($activities as $activity) {
                        if ($activity->get_index() == $value) {
                            $check = false;
                            break;
                        }
                    }
