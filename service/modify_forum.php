    ...
    if (isset($result['isValid']) && $result['isValid']) {
        try {
            ...
            foreach ($_POST as $key => $value) {
                if (array_key_exists($key, Forum::INPUTS_MAP)) {
                    $column = Forum::INPUTS_MAP[$key];
                } else {
                    continue;
                }
                if (strcmp($key, 'id') !== 0) {
            ...
            $sql = "UPDATE " . Forum::TABLE_NAME . " SET " . implode(", ", $columns_values) . " WHERE " . Forum::FIELDS_MAP['id'] . "=:" . Forum::FIELDS_MAP['id'] . ";";
        ...
        $json_response = ['error' =>  'Invalid forum'];
    }
} else {
